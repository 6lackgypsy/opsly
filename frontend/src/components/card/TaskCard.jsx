  ...
  return (
    ...
    >
      <div className="flex items-end gap-3 px-4">
        <div
          className={`rounded px-4 py-0.5 text-[11px] font-medium ${getStatusTagColor()}`}
        >
          {status}
        </div>
        <div
          className={`rounded px-4 py-0.5 text-[11px] font-medium ${getPriorityTagColor()}`}
        >
          {priority} Priority
        </div>
      </div>
      <div
        className={`border-l-[3px] px-4 ${status === "In Progress" ? "border-cyan-500" : status === "Completed" ? "border-indigo-500" : "border-violet-500"}`}
      >
